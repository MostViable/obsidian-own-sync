...
import { packetLimitFromCapabilities } from '../src/sync/capabilities.ts';
...
test('rejects incompatible server capabilities and caps the packet limit', () => {
  assert.equal(packetLimitFromCapabilities({
    protocol_version: 0, packet_format_version: 1, max_packet_bytes: 1024 * 1024,
  }), 1024 * 1024);
  assert.equal(packetLimitFromCapabilities({
    protocol_version: 0, packet_format_version: 1, max_packet_bytes: 1024 * 1024 * 1024,
  }), 64 * 1024 * 1024);
  assert.throws(() => packetLimitFromCapabilities({
...
test('binary packet round trips Cyrillic paths and deletion', async () => {
  ...
  const packet = await encodePacket(changes, LIMIT);
...
test('rejects corrupted content and duplicate paths', async () => {
  const packet = await encodePacket([{ path: 'a.md', kind: 'put', bytes: new Uint8Array([1]) }], LIMIT);
...
test('rejects unsupported versions, extra fields and oversized packets', async () => {
  const packet = await encodePacket([{ path: 'a.md', kind: 'delete' }], LIMIT);
  const encoded = JSON.parse(new TextDecoder().decode(packet));
